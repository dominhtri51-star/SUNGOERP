...
async function syncAll() {
    console.log("=== 1. ĐỒNG BỘ CẤU TRÚC BẢNG & CỘT TỪ LOCAL SANG CLOUD ===");
    ...
    for (const { table_name } of tablesRes.rows) {
        await cloudPool.query(`CREATE TABLE IF NOT EXISTS "${table_name}" (id SERIAL PRIMARY KEY)`).catch(() => {});
        ...
        for (const col of colsRes.rows) {
            ...
            if (typeStr === "USER-DEFINED") typeStr = col.udt_name;
            if (typeStr === "character varying") {
                typeStr = col.character_maximum_length ? `VARCHAR(${col.character_maximum_length})` : "VARCHAR(255)";
            }

            try {
                await cloudPool.query(`ALTER TABLE "${table_name}" ADD COLUMN IF NOT EXISTS "${col.column_name}" ${typeStr}`);
            } catch(colErr) {
                try {
                    await cloudPool.query(`ALTER TABLE "${table_name}" ADD COLUMN IF NOT EXISTS "${col.column_name}" TEXT`);
                } catch(e) {}
            }
        }
    }
    console.log("✅ Cấu trúc tất cả các bảng và cột đã hoàn toàn đồng bộ.");

    console.log("\n=== 2. DỌN DẸP DỮ LIỆU CŨ TRÊN CLOUD THEO RÀNG BUỘC ===");
    ...
    try {
        ...
        console.log("\n=== 3. NẠP DỮ LIỆU TOÀN BỘ TỪNG BẢNG TỪ LOCAL SANG CLOUD ===");
        for (const table of orderedTables) {
            try {
                ...
                console.error(`❌ Lỗi nạp bảng "${table}":`, tblErr.message);
            }
        }

        // Cập nhật lại SERIAL Sequences
        for (const table of orderedTables) {
            try {
        ...
        console.log("\n=== 4. THỐNG KÊ DỮ LIỆU TRÊN RENDER CLOUD SAU KHI ĐỒNG BỘ ===");
    ...
    console.log("\n🎉 HOÀN TẤT ĐỒNG BỘ DỮ LIỆU 100% LÊN RENDER CLOUD!");
}
